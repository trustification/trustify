...
The following Red Hat services can be used for the installation

* OIDC provider: Red Hat Single Sign-on Operator
* Storage: Red Hat Openshift Data Foundation Operator

=== Manual setup

==== Red Hat Single Sign-on operator

 * Install Single Sing-on operator with deployment https://docs.redhat.com/en/documentation/red_hat_single_sign-on/7.6/html/server_installation_and_configuration_guide/operator#installing-operator[guide]
 * Navigate to the related Keycloak instance of RHSSO operator and login to the Admin console with valid credentials

Follow the below steps required to configure Keycloak:
 ...
 * Create the following custom role: `trust-admin`. (Note: This role name can be customized to your specific needs).
 ...
 * Navigate to the **Clients** section in the Keycloak administration console.
 ...
 * Go to the **Role Mapping** tab for this user and assign the `trust-admin` role (created in <<_role_definition,Role Definition>>) to them.
...
For the TPA installation, the following OIDC (OpenID Connect) values are retrieved from your Keycloak (RH-SSO) configuration:

* **issuerURL**: `_keycloakURL_/realms/<<_realm_creation,Realm name>>`
* **frontend**: `empty object as {}` (This typically means no secret or specific configuration is needed for the frontend from the perspective of OIDC setup for the TPA install).
...
This guide details the steps to configure and verify the Red Hat OpenShift Data Foundation (ODF) Operator.
...
Before proceeding with the ODF installation, ensure you have the following:
...
[NOTE]
.Optional: Adding Additional Nodes for ODF
It's recommended to add additional worker nodes to your OpenShift cluster to avoid performance issues. Follow the below steps to add additional nodes
...
Follow these steps to install and configure the ODF Operator:

* Create a dedicated namespace for the ODF installation.
[source, bash]
...
* Install the ODF Operator by following the official deployment link:https://docs.redhat.com/en/documentation/red_hat_openshift_data_foundation/4.18/html-single/deploying_openshift_data_foundation_on_any_platform/index#deploy-standalone-multicloud-object-gateway[guide].

* Confirm the ODF installation is successful and the `StorageCluster` is in a `READY` state.
[source, bash]
  oc get storagecluster -n openshift-storage ocs-storagecluster -o jsonpath='{.status.phase}{"\n"}'

[[_object_storage]]
===== Object Storage Configuration
After ODF is installed, proceed with the following steps to configure and test object storage:
...
* Verify the connection to your S3 endpoint by exporting the ODF storage <<_object_storage, details>> and SSL <<_ocp_tls_certs, certs>> using the AWS CLI.
[source, bash]
...
[NOTE]
.Optional: Adding Bucket Policies
Bucket policies can be added to object buckets using the AWS CLI to manage access permissions.
...
===== Usage
For the TPA installation, the following S3 values are retrieved from your Red Hat OpenShift Data Foundation (RHODF) installation:

* **type**: `s3`
* **region**: The External DNS from the `noobaa status` command (refer to <<_retrieve_noobaa_credentials_and_endpoint,Object Storage Configuration and Testing>>)
* **bucket**: S3 Bucket created
* **accessKey**: AWS_ACCESS_KEY_ID 
* **secretKey**: AWS_SECRET_ACCESS_KEY

Additionally, the `tls.crt` file must be referred to for the installation with `Values.tls.additionalTrustAnchor`
