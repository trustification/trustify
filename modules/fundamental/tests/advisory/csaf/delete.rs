...
#[test_context(TrustifyContext)]
#[test(tokio::test)]
async fn delete_then_reingest(ctx: &TrustifyContext) -> anyhow::Result<()> {
    let r1 = ingest_advisory(ctx).await?;

    // now delete

    let service = AdvisoryService::new(ctx.db.clone());
    let advisory_details = service.fetch_advisory(r1.clone().id, &ctx.db)
        .await?;
    assert_eq!(advisory_details.unwrap().vulnerabilities.len(),1);
    service
        .delete_advisory(r1.id.try_as_uid().expect("must be a UUID variant"), &ctx.db)
        .await?;
    ...
    //check advisory doesn't exist any more
    assert_eq!(v.advisories.len(), 0);

    //reingest the same file
    let _r2 = ingest_advisory(ctx).await?;
