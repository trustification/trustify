...
begin
    ...
    if left_qualifier_or_build is null then
        -- if we have the build number, than we should treat
        -- versions with qualifiers as higher
        if array_length(left_parts, 1) > 3 then
            return -1;
        else
            return +1;
        end if;
    ...
    if right_qualifier_or_build is null then
        -- if we have the build number, than we should treat
        -- versions with qualifiers as higher
        if array_length(right_parts, 1) > 3 then
            return +1;
        else
            return -1;
        end if;
